...
import subprocess
import uuid
from car_chase_eval import car_chase_eval
from fast_rcnn.config import cfg


class car_chase(imdb):
    def __init__(self, image_set, devkit_path=None):
        imdb.__init__(self, 'car_chase' + '_' + image_set)
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(cfg.DATA_DIR, 'car_chase')
    ...
    def _do_python_eval(self, output_dir = 'output'):
        ...
        for i, cls in enumerate(self._classes):
            ...
            if (ap > 0):
                ...
                print('No detections of {}'.format(cls))
        print('Mean AP = {:.4f}'.format(np.mean(aps)))
        print('~~~~~~~~')
        print('AP Results:')
        for index, ap in enumerate(aps):
            print('{}: {:.3f}'.format(detected_classes[index], ap))
...
if __name__ == '__main__':
    from datasets.car_chase import car_chase
    d = car_chase('val')
    res = d.roidb
